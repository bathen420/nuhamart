...
class StoreOrderRequest extends FormRequest
{
    ...
    {
        $enabledPayments = array_keys(
            collect(config('commerce.payments', []))
        ...
        return [
            ...
            'division' => [
                'required_unless:shipping_method,store_pickup',
                'nullable',
                'string',
                'max:100',
            ],
            ...
            'shipping_method' => [
                'required',
                Rule::in(['standard', 'store_pickup']),
            ],
            'payment_method' => [
                'required',
                Rule::in($enabledPayments),
            ],

            'address_label' => [
                Rule::requiredIf(
                    fn () => $this->boolean('save_address')
                        && $this->input('shipping_method') !== 'store_pickup',
                ),
                'nullable',
                'string',
                'max:60',
    ...
    public function messages(): array
    {
        return [
            ...
            'address_label.required' => 'Add a label such as Home or Office.',
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'coupon_code' => $this->filled('coupon_code')
                ? strtoupper($this->input('coupon_code'))
                : null,
        ]);
    }
}
